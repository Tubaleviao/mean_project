...
router.post("/signup", async function(req, res) {
  ...
  const email_exist = await req.db
    .collection("users")
...
router.post("/signin", async function(req, res) {
  ...
  res
    ...
    .json({
      ...
      token: token,
      user_id: user._id,
      api_token: process.env.GOOGLE_KEY
    });
});
    ...
    .updateOne(
      ...
      (err, data) => {
        if (err) {
          res.json({
            success: false,
            message: "Friend not added! Error: " + err
          });
        } else {
          res.json({
            success: true,
            message: `Your friend, ${req.body.friend} is added to your friends list!`
          });
        }
      }
    );
